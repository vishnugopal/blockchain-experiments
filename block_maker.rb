...
class BlockMaker
  ...
  def mine_and_add_to_blockchain
    ...
    viable_hash = verify_hash(incoming_block[:random_seed], incoming_block[:transactions], 
  ...
  def verify_hash(random_seed, transactions, winning_hash)
    difficulty = 3
    hash = Digest::SHA2.hexdigest(Digest::SHA2.hexdigest("#{random_seed}#{transactions.to_json}"))
